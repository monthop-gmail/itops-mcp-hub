...
3. **PDF** อ่านด้วย `pdftotext -layout` และ `-raw` แล้วเลือกหน้าที่สระไทยหลุดน้อยกว่า แล้ว `normalize` (NFC + ติดสระลอยที่ถูกแยกด้วยช่องว่าง)
4. ค้นด้วย FTS5 ทั้งรูปปกติและรูปที่ตัดสระบน/ล่างออก — ชื่ออย่าง ประจวบคีรีขันธ์ / ครุภัณฑ์ ยังเจอแม้ต้นฉบับสกัดเป็น ประจวบคีรขี นั ธ์ / ครุภณั ฑ์
5. หน้าที่มีตัวอักษรที่มองเห็นได้น้อยกว่า `RAG_OCR_MIN_CHARS` (ค่าเริ่ม 40) **ไม่ถูกอินเด็กซ์ว่าง ๆ** — เข้าคิว OCR แทน
6. ไฟล์ที่ยังไม่รองรับ (docx/xlsx/รูปเดี่ยว) นับเป็น `skipped` ใน `rag_get_status` — แปลงเป็น PDF/ข้อความทีหลังได้โดยไม่ย้ายต้นฉบับ
...
หลังอัปเดตรอบสระไทย (fold + pdftotext raw) ต้อง **reindex** คลังจริงจึงจะค้นชื่อจังหวัด/ครุภัณฑ์แม่น — `rag_reindex` หรือ recreate `sub-mcp-rag`
